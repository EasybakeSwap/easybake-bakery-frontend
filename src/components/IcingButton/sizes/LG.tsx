...
const spinDownOn = keyframes`{
  0% {
    transform: translateX(-5%) translateY(-40%) rotate(0deg);
  }
  100% {
...
const spinDownOff = keyframes`{
  0% {
    transform: translateX(0%) translateY(50%) rotate(390deg);
  }
  100% {
...
const Base = styled.button<ButtonType>`
  ...
  height: 3rem;
  width: ${(props) => (props.scale ? `${props.scale}` : '12.25rem')};
  outline: 0;
  overflow: hidden;
...
const IcingButtonLG: React.FC<ButtonType> = ({
  btnName,
  isLoading,
  isDisabled,
  onClick,
...
const BaseButtonLG: React.FC<ButtonType> = ({
  scale,
  btnName,
  isLoading,
  isDisabled,
...
export { IcingButtonLG, BaseButtonLG };