...
export const fetchPoolsPublicDataAsync = (currentTime: number) => async (dispatch, getState) => {
  const timeLimits = await fetchPoolsTimeLimits()
  const totalStakings = await fetchPoolsTotalStaking()

  const prices = getTokenPricesFromFarm(getState().farms.data)

  const liveData = poolsConfig.map((pool) => {
    const blockLimit = timeLimits.find((entry) => entry.sousId === pool.sousId)
    const totalStaking = totalStakings.find((entry) => entry.sousId === pool.sousId)
    const isPoolEndBlockExceeded = currentTime > 0 && blockLimit ? currentTime > Number(blockLimit.endTime) : false
