...
export const getStaticPredictionsData = async () => {
  ...
  const [currentEpoch, intervalTimes, minBetAmount, isPaused, bufferTimes] = await makeBatchRequest([
    methods.currentEpoch().call,
    methods.intervalTimes().call,
    methods.minBetAmount().call,
    methods.paused().call,
    methods.bufferTimes().call,
  ])

  return {
    status: isPaused ? PredictionStatus.PAUSED : PredictionStatus.LIVE,
    currentEpoch: Number(currentEpoch),
    intervalTimes: Number(intervalTimes),
    bufferTimes: Number(bufferTimes),
    minBetAmount,
  }
}
