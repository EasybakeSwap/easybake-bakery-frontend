...
const Mint: React.FC = () => {
  ...
  const { actions, minimumOvenRequired, allowance } = useProfileCreation()

  const { account } = useWeb3React()
  const ovenContract = useOven()
  const bunnyFactoryContract = useBunnyFactory()
  const { t } = useTranslation()
  const hasMinimumCakeRequired = useHasOvenBalance(minimumCakeBalanceToMint)
  ...
  return (
    <>
      ...
      <Text as="p" mb="24px">
        {t('You can change your profile pic later if you get another approved EasyBake Collectible.')}
      </Text>
      <Card mb="24px">
        <CardBody>
