...
const UserName: React.FC = () => {
  ...
  const { teamId, selectedNft, userName, actions, minimumOvenRequired, allowance } = useProfileCreation()
  ...
  const hasMinimumCakeRequired = useHasOvenBalance(minimumCakeToRegister)
  const [onPresentConfirmProfileCreation] = useModal(
    <ConfirmProfileCreationModal
      ...
      account={account}
      teamId={teamId}
      minimumOvenRequired={minimumOvenRequired}
      allowance={allowance}
    />,
    false,
  )
  ...
  const handleConfirm = async () => {
    try {
      ...
      const signature = library?.ethSign
        ? (await library.ethSign(account, userName))?.signature
