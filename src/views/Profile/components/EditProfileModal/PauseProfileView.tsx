...
import { AutoRenewIcon, Button, Checkbox, Flex, InjectedModalProps, Text } from 'easybake-uikit'
...
const PauseProfilePage: React.FC<PauseProfilePageProps> = ({ onDismiss }) => {
  ...
  const { numberOvenToReactivate } = useGetProfileCosts()
  const { t } = useTranslation()
  const easybakeProfileContract = useProfileContract()
  ...
  const handleDeactivateProfile = () => {
    easybakeProfileContract.methods
      .pauseProfile()
      .send({ from: account })
      .on('sending', () => {
  ...
  return (
    <>
      ...
      <Text as="p" color="textSubtle" mb="24px">
        {t('Cost to reactivate in the future: %cost% CAKE', { cost: getBalanceNumber(numberOvenToReactivate) })}
      </Text>
