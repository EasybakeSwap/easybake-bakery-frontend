...
import useHasOvenBalance from 'hooks/useHasOvenBalance'
...
const StartPage: React.FC<StartPageProps> = ({ goToApprove, goToChange, goToRemove, onDismiss }) => {
  ...
  const minimumOvenRequired = profile.isActive ? numberOvenToUpdate : numberOvenToReactivate
  const hasMinimumCakeRequired = useHasOvenBalance(minimumOvenRequired)
  ...
  useEffect(() => {
    const checkApprovalStatus = async () => {
      const response = await ovenContract.methods.allowance(account, getEasyBakeProfileAddress()).call()
      ...
      <Flex alignItems="center" style={{ height: '48px' }} justifyContent="center">
        <Text as="p" color="failure">
          {!hasMinimumCakeRequired &&
            t('%minimum% CAKE required to change profile pic', { minimum: getFullDisplayBalance(minimumOvenRequired) })}
        </Text>
      </Flex>
      {profile.isActive ? (
        <>
