...
import { Button, InjectedModalProps, Skeleton, Text } from 'easybake-uikit'
...
const ChangeProfilePicPage: React.FC<ChangeProfilePicPageProps> = ({ onDismiss }) => {
  ...
  const { isApproving, isApproved, isConfirmed, isConfirming, handleApprove, handleConfirm } =
    useApproveConfirmTransaction({
      onApprove: () => {
        return contract.methods.approve(getEasyBakeProfileAddress(), selectedNft.tokenId).send({ from: account })
      },
      onConfirm: () => {
        if (!profile.isActive) {
