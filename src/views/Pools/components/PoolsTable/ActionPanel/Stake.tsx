...
import { Button, useModal, IconButton, AddIcon, MinusIcon, Skeleton, useTooltip, Flex, Text } from 'easybake-uikit'
...
import VaultStakeModal from '../../OvenVaultCard/VaultStakeModal'
...
const Staked: React.FunctionComponent<StackedActionProps> = ({ pool, userDataLoaded }) => {
  ...
  const stakedAutoDollarValue = getBalanceNumber(ovenAsBigNumber.multipliedBy(stakingTokenPrice), stakingToken.decimals)

  const needsApproval = isAutoVault ? !isVaultApproved : !allowance.gt(0) && !isEthPool
  ...
  const [onPresentVaultUnstake] = useModal(<VaultStakeModal stakingMax={ovenAsBigNumber} pool={pool} isRemovingStake />)
        ...
        <ActionContent>
          <Flex flex="1" pt="16px" flexDirection="column" alignSelf="flex-start">
            <Balance
              ...
              fontSize="20px"
              decimals={5}
              value={isAutoVault ? ovenAsNumberBalance : stakedTokenBalance}
            />
            <Balance
              fontSize="12px"
              display="inline"
