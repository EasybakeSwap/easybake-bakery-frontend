...
const ActionPanel: React.FC<ActionPanelProps> = ({ account, pool, userDataLoaded, expanded, breakpoints }) => {
  const { sousId, stakingToken, earningToken, totalStaked, endTime, stakingLimit, isAutoVault } = pool
  ...
  } = useTooltip(('Total amount of %symbol% staked in this pool' + { symbol: stakingToken.symbol }), {
    placement: 'bottom',
  })
  ...
  const blocksRow =
    timeRemaining || timeUntilStart ? (
      <Flex mb="8px" justifyContent="space-between">
        ...
        <Flex>
          <Link external href={getErcScanBlockCountdownUrl(endTime)}>
      ...
      <ActionContainer>
        {showSubtitle && (
          <Text mt="4px" mb="16px" color="textSubtle">
            {isAutoVault ? ('Automatic restaking') : `${('Earn')} OVEN ${('Stake').toLocaleLowerCase()} OVEN`}
          </Text>
        )}
